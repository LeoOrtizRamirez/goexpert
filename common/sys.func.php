...
function time_expiry($sestime, $isminsec = 0) {
    ...
    if ($sestime > $cfgrow['datetimestr']) {
        ...
        if ($diff->h) {
            $result .= ' and ' . $diff->h . ($diff->h > 1 ? ' hours ' : ' hour ');
        }
...
function getcmlist($sprstr, $sprlist, $cmlist, $mbrstr = array(), $trxstr) {
    ...
    if ((in_array($mbrstr['mppid'], $sprppstr['pparr_all']) && $sprppstr['mpstatus'] == 1) || $frlmtdcfg['isregallrefs'] == 1) {
        ...
        for ($i = 0; $i < $mpdepth; $i++) {
            ...
            $sprcm = getamount($cmlistarr[$i], $regnow_fee);
...
function addcmlist($memo, $tokencode, $valcmlist = array(), $mbrstr = array(), $trxstr = array()) {
...
function dolvldone($mbrstr, $trxstr, $mppid = 1) {
    ...
    $dirsprstr = getmbrinfo($mbrstr['idspr']);
    $mpidspr = ($dirsprstr['mppid'] > $mbrstr['mppid']) ? $mbrstr['mppid'] : $dirsprstr['mppid'];

    $rwlist = (defined('ISCALC_BYMBR')) ? $bpparr[$mbrstr['mppid']]['rwlist'] : $bpparr[$mpidspr]['rwlist'];
...
function do_reginorder($mbrstr) {
    ...
    $nextppid = 0;
    if ($frlmtdcfg['isreginorder'] != 1) {
        return $nextppid;
    } else {
        $mbrlistpparr = mbrpparr($mbrstr['id']);
        foreach ($bpparr as $key => $value) {
            if ($mbrlistpparr[$value['ppid']]['mpid'] > 0) {
                $nextppid = $value['ppid'] + 1;
                break;
            } else {
                continue;
            }
